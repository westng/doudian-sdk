...
class DouDianOpSpiClient
{
    public function request($request, $callback)
    {
        ...
        $sortedParamJson = \DouDianSdk\Utils\SignUtil::marshal($paramJsonArray);
        $signMethodNumber = $signMethod == 'hmac-sha256' ? 2 : 1;
        //验证签名
        $calcSign = \DouDianSdk\Utils\SignUtil::spiSign($appKey, $appSecret, $timestamp, $sortedParamJson, $signMethodNumber);
        print $calcSign . "\n";
        if ($sign != $calcSign) {
